/**
 * 從 Google 行事曆內文解析實際收入金額（瀏覽器版）。
 * 與 functions/_lib/stated-price.js 規則保持一致。
 */
(function (global) {
/**
 ...
 * 若有 = 則取右側數字。
 */
function evaluateMoneyExpression(raw, minAmount = 1000) {
  if (!raw) return null;
...
function isPrivateBookingText(text) {
  if (!text) return false;
  return /私[訂定]|私下|(?:^|[\s　])私(?:$|[\s　])/.test(text);
}

  global.JoyforestStatedPrice = {
    evaluateMoneyExpression: evaluateMoneyExpression,
    parseStatedPriceFromText: parseStatedPriceFromText,
    isPrivateBookingText: isPrivateBookingText
  };
})(window);
